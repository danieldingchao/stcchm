...
int DES::ydecrypt ( unsigned char key[8], unsigned char* data, int blocks, int* size )
{
  ...
  {
    int pos = blocks*8-1;
    unsigned char endChar = data[pos];
    while ((pos>0) && (data[pos]==endChar)) {
        pos--;
    }
    if ( data[pos] != (unsigned char)~endChar ) {
      return 0;
    }
    *size = pos+1;
  }
...
bool des_encrypt(unsigned char key[8], const std::string& input, std::string* output)
{
  if (input.empty())
	  return false;
  int size = (input.size()/8+1)*8;
  char *buffer = (char *)calloc(size, 1);
  if (buffer == NULL)
	  return false;
  memcpy(buffer, input.c_str(), input.size());
  DES des;
  des.yencrypt(key, (unsigned char *)buffer, input.size());
  base::Base64Encode(std::string(buffer, size), output);
  free(buffer);
  return true;
}

bool des_decrypt(unsigned char key[8], const std::string& input, std::string* output)
{
  if (input.empty())
	return false;
  base::Base64Decode(input, output);
  std::string& s = *output;
  int buffer_size = s.size();
  ...
  DES des;
  des.ydecrypt(key, (unsigned char*)buffer, buffer_size/8, &buffer_size);
